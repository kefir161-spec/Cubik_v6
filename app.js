...
// ===== Globals =====
  ...
  var MAX_UNDO_STEPS = 600; // maximum steps remembered in history
...
function aabb(kind,center){
  ...
  function canPlace(center, kind, ignore){
    ...
    // Чуть сжимаем тестовый бокс, чтобы избежать ложных пересечений.
    // Для Zen/2 уменьшаем X/Z посильнее, а Y совсем немного —
    // так верх/низ продолжает стыковаться по границе, как в нуле,
    // а вот по бокам можно чуть "поджаться" между колоннами.
    var rx, ry, rz;
    if (isZen2LikeKind(kind)){
      rx = 0.94;
      ry = 0.995;
    ...
    var hsh=new THREE.Vector3(h.x*rx, h.y*ry, h.z*rz);
    ...
    for (var i=0;i<objects.length;i++){
      ...
      var ob;
      // Для Zen/2-подобных блоков берём реальный AABB по текущему повороту меша,
      // а не через aabb()/getHalf(), которые завязаны на глобальный zen2OrientationIndex.
...
function snap(v, step){
  ...
  function onMove(e){
    ...
    if(hit.object===ground){
      ...
      // Для Zen/2-подобных блоков при наведении на пол не ограничиваем
      // размещение пересечениями с другими кубами: разрешаем ставить как
      // угодно по полу, пока нижняя грань не уходит ниже уровня пола.
      if (isZen2LikeKind(ghostType)){
        ok = (pos.y - h.y >= 0);
      }
    }else {
      ...
      // Если Zen/2-подобный куб и он стоит на полу
      // (нижняя грань примерно на уровне y=0), то по просьбе автора
      // разрешаем любое расположение по X/Z при стыковке к блоку.
      // Ограничения коллизий остаются только по высоте (не ниже пола).
      if (isZen2LikeKind(ghostType)){
        var epsFloor = Math.max(h.x,h.y,h.z) * 1e-3;
        if (pos.y - h.y <= epsFloor){
          ok = true;
        }
      }
...
// ===== Materials / block constructors =====
  ...
  function ensurePreview(){
    ...
    if(!previewScene){
      ...
      previewScene.background=new THREE.Color(0x8B95A7);
      ...
      if(s.type === 'solid'){
        ...
        for(var dir in s.faces){
          ...
          if(faceMesh){
            ...
            existingObj.userData.faceTypes[dir] = fs.faceType;
          }
        }
      }
...
// Helper: set UI disabled/enabled while timelapse replay is active
function setReplayUIBusy(busy){
  var ids = ['clearBtn','undoBtn','redoBtn','replayBuildBtn','recordTimelapseBtn'];
...
// Start recording canvas to WebM (timelapse)
function startCanvasRecording(){
  if (!renderer || !renderer.domElement || typeof renderer.domElement.captureStream !== 'function'){
    msg('Запись видео не поддерживается в этом браузере', true);
    return false;
  }
  recordedChunks = [];
  var stream = renderer.domElement.captureStream(60);
  try{
...
// Replay build history based on undoStack. If options.record === true, also record video.
function startBuildReplay(options){
  ...
  // final state = last snapshot (current scene)
  ...
  // start immediately
  isReplayingBuild = true;
  step();
  replayTimer = setInterval(step, 80); // speed of timelapse
}
